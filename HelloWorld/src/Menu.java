...
public class Menu {
	...
	static void ajustePreco(){
		...
		do {
			...
			valor = Double.parseDouble(JOptionPane.showInputDialog("Digite o valor do produto ou 0 (n�mero zero) para sair: "));
			if(valor == 0)
				return;
			
			fator = JOptionPane.showInputDialog("1 - 5% de desconto\n2 - 7% de desconto\n3 - 10% de desconto\n"
					+ "4 - 11% de desconto\n5 - 20% de desconto\n\nDigite o ID referencia para gerar Desconto:\n\nou 0 (n�mero zero) para sair: ");
	...
	static void mediaFatec(){
		...
		if(media >= 6.0){
			...
			if(mediaF >= 6.0){
				JOptionPane.showMessageDialog(null, "M�dia Final = "+mediaF+".\n\nAprovado!");
			}else
	...
	static void fatorialFor(){
		...
		fator = Integer.parseInt(JOptionPane.showInputDialog(""
				+ "Digite o n� que deseja saber o valor Fatorial:"));
		
		result = fator;
		for (int i = fator; i > 1; i--) {
			result = result * (i - 1);
		}
		JOptionPane.showMessageDialog(null, fator+"! � igual a "+result+".");
	}
	...
	static void fibonacciFor(){
		...
		JOptionPane.showMessageDialog(null, "O " +termo+"� termo fibonacci � "+aux2);
	}
